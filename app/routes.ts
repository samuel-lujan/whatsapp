...
export function createRouter(rollbar: Rollbar): Router {
    ...
    try {
      ...
      if (whatsapp.hasActiveSession(companySlug)) {
        ...
        if (quickStatus.connected) {
          console.log(
            `⚡ Empresa ${companySlug} já conectada (verificação rápida)`,
          );
          return res.json({
            connected: true,
            companySlug,
      ...
      if (status.connected) {
        ...
        if (status.qrCode) {
          ...
          response.message =
            status.message || "Aguardando geração do QR Code...";
        }
      ...
      console.error(
        `❌ Erro ao verificar status da empresa ${companySlug}:`,
        err.message,
      );
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        console.log(
          `🔍 Verificando conexão da empresa ${companySlug} antes de enviar...`,
        );
        ...
        console.error(
          `❌ Erro ao enviar mensagem pela empresa ${companySlug}:`,
          err.message,
        );
        rollbar.error(err, {
          companySlug,
        ...
        const shouldRetry =
          err.shouldRetry !== undefined ? err.shouldRetry : false;
  ...
  router.get(
    "/search-number/:companySlug/:number",
    authenticateToken,
    async (req, res) => {
      ...
      try {
        console.log(
          `🔍 Buscando informações do número ${number} para empresa ${companySlug}`,
        );
        ...
        console.error(
          `❌ Erro ao buscar informações do número ${number}:`,
          error.message,
        );
        res.status(500).json({
          error: error.message,
  ...
  router.delete("/clear/:companySlug", authenticateToken, async (req, res) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          `⚠️ Falha ao limpar sessão ${companySlug}:`,
          result.message,
        );
        res.status(404).json({
          success: false,
  ...
  router.delete("/delete-all", authenticateToken, async (req, res) => {
    try {
      ...
      console.error(
        `❌ Erro ao deletar todas as empresas e sessões:`,
        error.message,
      );
