...
import { safeDestroyClient, scheduleReconnect } from "./lifecycle";
import { sessions } from "./sessions";
import { raceWithTimeout } from "../../utils";

export async function verifyClientHealth(
  companySlug: string,
  ...
  try {
    if (client.pupPage) {
      try {
        ...
        if (isClosed) {
          ...
          return {
            healthy: false,
            reason: "Página do browser fechada",
    ...
    if (state && disconnectedStates.includes(state as string)) {
      ...
      return {
        healthy: false,
        reason: `Estado de desconexão: ${state}`,
        shouldReconnect: true,
      };
    }
...
async function zombieSessionMonitor(): Promise<void> {
  ...
  for (const slug of slugs) {
    ...
    try {
      ...
      console.log(
        `[HEALTH] ${slug}: erro durante verificacao: ${(err as Error).message}`,
      );
    }
  }
}
